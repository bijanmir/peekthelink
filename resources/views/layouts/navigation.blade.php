        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center">
                        ...
                        <span class="font-bold text-xl text-gray-900">PeekTheLink</span>
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                <a href="{{ route('profile.show', Auth::user()->username) }}" 
                   target="_blank"
                   class="text-gray-500 hover:text-gray-700 px-3 py-2 rounded-md text-sm font-medium flex items-center mr-4">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center">
                                ...
                                @else
                                    <div class="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold mr-2">
                                        {{ strtoupper(substr(Auth::user()->display_name ?? Auth::user()->name, 0, 1)) }}
                                    </div>
                                @endif
                                <span>{{ Auth::user()->display_name ?? Auth::user()->name }}</span>
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-2 border-b border-gray-100">
                            <div class="text-sm text-gray-900 font-medium">{{ Auth::user()->display_name ?? Auth::user()->name }}</div>
                        ...
                        <x-dropdown-link :href="route('profile.show', Auth::user()->username)" target="_blank">
                            <div class="flex items-center">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-2M14 6h6m0 0v6m0-6L10 16"></path>
                                </svg>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4 flex items-center">
                ...
                @endif
                <div>
                    <div class="font-medium text-base text-gray-800">{{ Auth::user()->display_name ?? Auth::user()->name }}</div>
            ...
            <div class="mt-3 space-y-1">
                ...
                <x-responsive-nav-link :href="route('profile.show', Auth::user()->username)" target="_blank">
                    {{ __('View Public Profile') }}
                </x-responsive-nav-link>
